...
export const updateModelCenterSchema = z
    ...
    .refine((value) => new Set(value.agents.map((item) => item.agentName)).size === value.agents.length, "Agent overrides must be unique.")
    .refine((value) => new Set(value.models.map((item) => item.id)).size === value.models.length, "Saved model ids must be unique.")
    .refine((value) => new Set(value.agentBindings.map((item) => item.agentName)).size === value.agentBindings.length, "Agent bindings must be unique.");
...
export const videoAssetTypeSchema = z.enum(["voice", "image", "chart", "text_card", "subtitle", "video", "composition"]);
...
const zhihuNoteAgentDraftSchema = z.object({
    ...
    samplePreview: z.array(z.object({
